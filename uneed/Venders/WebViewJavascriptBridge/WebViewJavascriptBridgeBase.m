...
- (NSDictionary*)getDict {
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:self.data, @"data", [NSNumber numberWithBool:self.success], @"success", [NSNumber numberWithInteger:self.code], @"code", nil];
    return dict;
}
@end
...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (WVJBMessage* message in messages) {
        ...
        if (responseId) {
            ...
            if (callbackId) {
                responseCallback = ^(WVJBResponse *responseData) {
                    if (responseData == nil) {
                        responseData = [WVJBResponse new];
                        responseData.success = NO;
                        responseData.code = WVJBStatusCode_INTERNAL_ERROR;
                    } else if(![responseData isKindOfClass:[WVJBResponse class]]) {
                        id data = responseData;
                        responseData = [WVJBResponse new];
                        responseData.data = data;
                        responseData.success = YES;
                        responseData.code = WVJBStatusCode_OK;
                    }
                    WVJBMessage* msg = @{ @"responseId":callbackId, @"responseData":[responseData getDict] };
...
-(NSString *)webViewJavascriptCheckCommand {
    return @"typeof WBMovieJSBridge == \'object\';";
}

-(NSString *)webViewJavascriptFetchQueyCommand {
    return @"WBMovieJSBridge._fetchQueue();";
}
...
- (void)_dispatchMessage:(WVJBMessage*)message {
    ...
    NSString* javascriptCommand = [NSString stringWithFormat:@"WBMovieJSBridge._handleMessageFromObjC('%@');", messageJSON];
